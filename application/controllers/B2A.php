...
class B2A extends CI_Controller {
    ...
    public function get_b2a_form_view(){
		...
		if($admin_session['isLoggedIn'] && $this->input->is_ajax_request()){
            ...
            if(isset($inputData['user_id']) && !empty($inputData['user_id'])){
				$data['b2a'] = $this->auth_model->get_all_users($inputData['user_id']);
			}
			$view 		= $this->load->view('admin/b2a/b2a-form',$data, true);
	...
	public function set_b2a_display(){ 
