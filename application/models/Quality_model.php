...
class Quality_model extends MY_Model {
	...
	protected $table_name 	= 'tbl_quality';
	protected $primary_key 	= 'quality_id';
	
	public function __construct(){
		parent::__construct();
	}

    public function get_quality($quality_id = false, $display = '')
    {
        $this->db->select('tq.*');
        $this->db->from('tbl_quality tq');
        if(isset($display) && !empty($display)){
            $this->db->where('tq.display', $display);
        }
        if(isset($quality_id) && !empty($quality_id)){
