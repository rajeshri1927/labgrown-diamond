...
class Permanentpage_Model extends MY_Model {
...
/*******************************  permanent_landing_page  ****************************************************** */	
 ...
 /***********
  fn : getPermanentProductPageDetails based on requested pageno & page order -- gen
	...
	public function getPermanentLandingPageConfig($pid){ 
		$this->db->select('*');
        $this->db->from('tbl_permanent_landing_pages');
        $this->db->where('permanent_landing_page_id', $pid);
        $query = $this->db->get();
		//echo "lstQrt - ". $this->db->last_query();
        return $query->result_array();
	}
	
	public function getPermanentLandingPageDetails2($pid, $LpconfigAry){ 
	
	    $joinCondition="";
	    if($LpconfigAry['shape']!="")
			$joinCondition.= "plp.permanent_landing_page_shapes = ppp.permanent_product_page_shapes and ";
		if($LpconfigAry['clarity']!="")
			$joinCondition.= "plp.permanent_landing_page_clarity = ppp.permanent_product_page_clarity and ";
		if($LpconfigAry['color']!="")
			$joinCondition.= "plp.permanent_landing_page_color = ppp.permanent_product_page_color and ";
		if($LpconfigAry['cut']!="")
			$joinCondition.= "plp.permanent_landing_page_cut =  ppp.permanent_product_page_cut ";
		if($LpconfigAry['labs']!="")
			$joinCondition.= " and plp.permanent_landing_page_labs = ppp.permanent_product_page_labs'";
	
		$this->db->select('ppp.*,  tbl_rapaport.stone_amt');
        $this->db->from('tbl_permanent_product_page ppp');
		$this->db->join('tbl_rapaport', 'tbl_rapaport.stone_clarity = ppp.permanent_product_page_clarity and tbl_rapaport.stone_color = ppp.permanent_product_page_color');
        //$this->db->where('plp.permanent_landing_page_id', $pid);
		//$this->db->where('ppp', $pid);
		$this->db->where('tbl_rapaport.stone_shape', 'BR');
		
		if($LpconfigAry['size_from']!=""){
			$this->db->where('ppp.permanent_product_page_size_from  >=', $LpconfigAry['size_from']);
			$this->db->where('ppp.permanent_product_page_size_to  <=', $LpconfigAry['size_to']);
			
			$this->db->where('tbl_rapaport.stone_weight_frm >=', $LpconfigAry['size_from']);
			$this->db->where('tbl_rapaport.stone_weight_to >=', $LpconfigAry['size_from']);
	        $this->db->where('tbl_rapaport.stone_weight_frm<=', $LpconfigAry['size_to']);
			$this->db->where('tbl_rapaport.stone_weight_to>=', $LpconfigAry['size_to']);
		}
		if($LpconfigAry['size_to']!=""){
			//$this->db->where('permanent_product_page_size_to >=', $LpconfigAry['size_from']);
			//$this->db->where('permanent_product_page_size_to <=', $LpconfigAry['size_to']);
		}
		
		if($LpconfigAry['shape']!="")
			$this->db->where('ppp.permanent_product_page_shapes', $LpconfigAry['shape']);
		if($LpconfigAry['clarity']!="")
			$this->db->where('ppp.permanent_product_page_clarity',$LpconfigAry['clarity']);
		if($LpconfigAry['color']!="")
			$this->db->where('ppp.permanent_product_page_color',$LpconfigAry['color']);
		if($LpconfigAry['cut']!="")
			$this->db->where('ppp.permanent_product_page_cut',$LpconfigAry['cut']);
		if($LpconfigAry['labs']!="")
			$this->db->where('ppp.permanent_product_page_labs',$LpconfigAry['labs']);
		
		//$this->db->where('ppp.permanent_product_page_shapes','tbl_rapaport.stone_shape');
		//$this->db->where('ppp.permanent_product_page_size_from >=', 'tbl_rapaport.stone_shape.stone_weight_frm');
	    //$this->db->where('ppp.permanent_product_page_size_to <=', 'tbl_rapaport.stone_weight_to');
	
		$query = $this->db->get();
		$result = $query->result_array();
		echo "lstQrt - ". $this->db->last_query()."<br/><br/>";
		return $result;
	}
	...
	public function getPermanentPageBlocksForPermanentLandingPage($permanentLandingPageBlockProductId){
		$this->db->select('*');
        $this->db->from('tbl_permanent_landing_page_blocks');
		$this->db->join('tbl_permanent_product_page', '(tbl_permanent_product_page.permanent_product_page_filter1 = tbl_permanent_landing_page_blocks.permanent_landing_page_block_filter1 and tbl_permanent_product_page.permanent_product_page_filter2 = tbl_permanent_landing_page_blocks.permanent_landing_page_block_filter2 and tbl_permanent_product_page.permanent_product_page_filter3 = tbl_permanent_landing_page_blocks.permanent_landing_page_block_filter3  and tbl_permanent_product_page.permanent_product_page_filter4 = tbl_permanent_landing_page_blocks.permanent_landing_page_block_filter4  and tbl_permanent_product_page.permanent_product_page_filter5 = tbl_permanent_landing_page_blocks.permanent_landing_page_block_filter5)');
       
	   /*
	     
		 
	   /* $this->db->where('tbl_permanent_product_page.permanent_product_page_filter2', 'tbl_permanent_landing_page_blocks.permanent_landing_page_block_filter2');
		$this->db->where('tbl_permanent_product_page.permanent_product_page_filter3', 'tbl_permanent_landing_page_blocks.permanent_landing_page_block_filter3');
		$this->db->where('tbl_permanent_product_page.permanent_product_page_filter4', 'tbl_permanent_landing_page_blocks.permanent_landing_page_block_filter4');
		$this->db->where('tbl_permanent_product_page.permanent_product_page_filter5', 'tbl_permanent_landing_page_blocks.permanent_landing_page_block_filter5');
		$this->db->where('tbl_permanent_product_page.permanent_product_page_filter6', 'tbl_permanent_landing_page_blocks.permanent_landing_page_block_filter6');
		$this->db->where('tbl_permanent_product_page.permanent_product_page_filter7', 'tbl_permanent_landing_page_blocks.permanent_landing_page_block_filter7');
		$this->db->where('tbl_permanent_product_page.permanent_product_page_filter8', 'tbl_permanent_landing_page_blocks.permanent_landing_page_block_filter8');
		$this->db->where('tbl_permanent_product_page.permanent_product_page_filter9', 'tbl_permanent_landing_page_blocks.permanent_landing_page_block_filter9');
      */       
	    $this->db->where('tbl_permanent_landing_page_blocks.permanent_landing_page_FId',$permanentLandingPageBlockProductId);
	 	$this->db->limit('tbl_permanent_landing_page_blocks.permanent_landing_page_block_max_products'); 
        $query = $this->db->get();
		echo "<br/>Qry-".$this->db->last_query()."<br/>";
		$rowData = $query->result_array();
	    return $rowData;
	}
	
	public function get_permanent_landingpageConfig_list($category_id = ''){
        $this->db->select('*');
        $this->db->from('tbl_permanent_landing_page_config');
        $query = $this->db->get();
		//echo $query->num_rows();
        if($query->num_rows()){
            if(isset($category_id) && !empty($category_id)){
                ...
                return  array('state'=>TRUE,'title'=>'','type'=>'success','message'=>'product available.','data'=>$query->result_array());    
            }
        } else{
            return FALSE;
        }
