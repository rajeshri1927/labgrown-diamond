$(document).ready(function(){
    ...
    function getSubCategoryFrom(sub_category_id = null){
    	let endpoint = 'get-tax-form-view';
    	let formdata = {
    	...
    	$.Utility.getData(endpoint, formdata, (response) => {
    		...
    		if(response.state){
    			response.data.forEach((element)=>{
    				tbl_body += `
    					<tr>
							<td class="text-center">${element.tax_id}</td>
							<td class="text-center">${element.country_name}(${element.sortname})</td>
							<td class="text-center">${element.import_tax}%</td>
							<td class="text-center">${element.local_tax_title1}-${element.local_tax_value1}%</td>
							<td class="text-center">${element.local_tax_title2}-${element.local_tax_value2}%</td>
							<td class="text-center">${element.other_tax_title}-${element.other_tax_value}%</td>
							 
							<td class="text-center">
								<label class="switch switch-default switch-pill switch-primary">
                    				<input type="checkbox" class="switch-input" ${(element.display==='Y')?'checked="checked"':''} data-id="${element.b2b_ctry_priceRange_class_markup_id}">
                    				<span class="switch-label"></span>
                    				<span class="switch-handle"></span>
                  				</label>
							</td>
							<td class="text-center">
								<a href="javascript:void(0);" class="btn btn-link btn-action" data-action="edit" data-id="${element.b2b_ctry_priceRange_class_markup_id}"> Edit </a> 
								<a href="javascript:void(0);" class="btn btn-link btn-action" data-action="delete" data-id="${element.b2b_ctry_priceRange_class_markup_id}"> Delete </a>
							</td>
						</tr>
    				`
    			});
