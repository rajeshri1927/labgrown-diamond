(function($) {
    ...
    function getB2Cform(user_id = null){
    	let endpoint = 'get-b2c-form-view';
    	let formdata = {
    	...
    	$.Utility.getData(endpoint,formdata, (response) => {
    		...
    		if(response.state){
				...
				let b2c_form       = document.getElementById('b2c-form');	
    			...
    			if(user_id != null){
    				$('#card-title').text('Update B2C');
    				$('.btn-form-submit').text('Update');
    			} else {
    				$('#card-title').text('Add B2C');
    				$('.btn-form-submit').text('Save');
    			}
    			b2c_form.addEventListener('submit', setRoleDetails);
				$('.toggle-form').on('click', function(){
					if ($('.form-view').is(':visible')) {
	...
	function getB2C(){
		...
		let b2c_list_table = document.getElementById('b2c-list'); 
		let endpoint = 'get-all-b2c';
    	let formdata = {};
    	let tbl_body = '';
		if ($.fn.DataTable.isDataTable(b2c_list_table)) {
			$(b2c_list_table).DataTable().destroy();
		}
    	$.Utility.getData(endpoint, formdata, (response) => {
    		response = JSON.parse(response);
    		if(response.state){
    			...
    			if (table_body !== null) {
        			...
        			$(table_body).on('change', '.switch-input', (e)=>{
        				...
        				$.Utility.postData('set-b2c-display',params,function(response){
    						response = JSON.parse(response);
    						if(response.state){
    							$.Utility.notify(response);
    							getB2C();
    			...
    			tbl_body += `
    					<tr>
							<td class="text-center" colspan="7">
								<strong>There are no B2C added yet.</strong>
							</td>
						</tr>
    				`
				table_body.innerHTML = tbl_body;
		...
		bootbox.confirm({ 
		    ...
		    callback: function (result) {
		       if(result){
		       		...
		       		$.Utility.postData('delete-b2c',params,function(response){
						response = JSON.parse(response);
						if(response.state){
							$.Utility.notify(response);
							getB2C();
