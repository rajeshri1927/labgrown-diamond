(($) => {
    	...
    	$.Utility.getData(endpoint, formdata, (response) => {
    		...
    		if(response.state){
    			response.data.forEach((element)=>{
    				tbl_body += `
    					<tr>
    					    <td><img alt="Lab Grown Diamond" class="img-responsive" src="./assets/uploads/products/${element.system_file_name}" title="Lab Grown Diamond Admin Panel" style="height: 100px;"></td>
							<td>
								<strong>${element.product_title}</strong>
								<div>${element.category_title}</div>
							</td>
							<td class="text-center">${element.product_actual_price}</td>
							<td class="text-center">${element.product_discount_price}</td>
	...
	function uploadProductExcel() {
		alert("Hi");
		let endpoint = 'upload-product-excel';
