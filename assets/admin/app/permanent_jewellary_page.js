(($) => {
	...
	function getJewellary(){
		...
		let product_list_table = document.getElementById('jewellary-list'); 
    			...
    			tbl_body += `
    					<tr>
							<td class="text-center" colspan="6">
								<strong>There are no jewellary added yet.</strong>
							</td>
						</tr>
    				`
    			table_body.innerHTML = tbl_body;
	...
	$('#jewellary_excel').on('click',(e)=>{
		e.stopImmediatePropagation();
		e.preventDefault();
		uploadProductExcel();
	});
//upload jewellary data
	function uploadProductExcel(){
		alert("jewellary page");
		let endpoint = 'upload-Permanent-Jewellary-Page-excel';
		let formArray = $('input[name="jewellary_upload_excel"]').serializeArray();
		let formData = new FormData();
		...
		var modalContent = '<div class="mb-3">' +
			'<label for="jewellary_upload_excel">Upload File:</label>' +
			'<input type="file" id="jewellary_upload_excel" accept=".csv" name="jewellary_upload_excel" class="form-control">' +
			'</div>';
	
		bootbox.confirm({
			title: "Upload Permanent Jewellary Page Excel",
			message: modalContent,
			onEscape: false,
			backdrop: 'static',
			centerVertical: true,
			className: "jewellary-upload-excel-modal",
			buttons: {
				confirm: {
					label: 'Submit',
					className: 'btn-square btn-primary'
			...
			callback: function (result){
				if (result) {
					...
					var uploadedFile = $('#jewellary_upload_excel')[0].files[0];
					formData.append('jewellary_upload_excel', uploadedFile);
