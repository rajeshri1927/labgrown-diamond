(($) => {
	...
	function getProducts() {
		...
		let product_list_table = document.getElementById('product-list'); 
		let endpoint = 'get_jewellary_product_page_list';
		let formdata = {};
		let tbl_body = '';
		...
		$.Utility.getData(endpoint, formdata, (response) => {
			response = JSON.parse(response);
			if (response.state) {
				// Sort the data based on product_id in ascending order
				response.data.sort((a, b) => a.product_id - b.product_id);
				
				// Initialize a counter for sequential product IDs
				let sequentialId = 1;
				...
				response.data.forEach((element) => {
					...
					tbl_body += `
						<tr>
							...
							<td>${element.parent_image_SKU_ID}</td>
							<td class="text-center">
								<a href='get_jewellary_product_page_details/${element.parent_image_SKU_ID}' target=_blank> View </a> 
				...
				setTimeout(() => {
					$(product_list_table).DataTable({
						"aaSorting": [
							[0, 'asc']  // Sorting by the sequential ID column in ascending order
						]
					});
				}, 100);
			} else {
	...
	function uploadProductExcel(){
		let endpoint = 'upload-jewellary-product-Page-excel';
