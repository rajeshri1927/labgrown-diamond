$(document).ready(function(){
	...
	$(categoryFormElem).on('submit', function(event){
		...
		if(formValid){
			...
			var endpoint 	= 'set-jewellary-category';
		...
		bootbox.confirm({ 
		    ...
		    callback: function (result) {
		       if(result){
		       		...
		       		$.Utility.postData('delete-jewellary-category',params,function(response){
						response = JSON.parse(response);
						if(response.state){
							$.Utility.notify(response);
							initTable();
	...
	function getCategory(categoryId){
		var endpoint 	= 'get-jewellary-category';
		var params 		= {
			category_id : categoryId,
		}
